...
class PointClusteringHelper(points: FloatBuffer) {
    ...
    private fun allocateGrid(points: FloatBuffer) {
        ...
        val numCellsX = max(
            1.0, ceil(((bounds.maxX - bounds.minX) / GRID_CELL_SIZE).toDouble())
                .toInt().toDouble()
        ).toInt()
        val numCellsY = max(
            1.0, ceil(((bounds.maxY - bounds.minY) / GRID_CELL_SIZE).toDouble())
                .toInt().toDouble()
        ).toInt()
        val numCellsZ = max(
            1.0, ceil(((bounds.maxZ - bounds.minZ) / GRID_CELL_SIZE).toDouble())
                .toInt().toDouble()
        ).toInt()
